...
def baseline_LSTM(sequences_length_for_training, embedding_dim, embedding_matrix, vocab_size):
    ...
    lstm_encoder = LSTM(600, input_shape=(20, GLOVE_EMBEDDING_DIM))
    lstm_out = lstm_encoder(embedded_input_main)

    encode_mid_drop = Dropout(0.2)(lstm_out)

    decoded = Dense(200, name='decoded')(encode_mid_drop)
...
if __name__ == "__main__":
    ...
    if LOAD_SAVED_MODEL_AND_CONTINUE_TRAIN:
        ...
        model = baseline_LSTM(-1, EMBEDDING_DIM, embedding_W,  len(dictionary_object.word2id_dic))
